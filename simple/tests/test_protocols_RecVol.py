'''
Created on Sep 28, 2018

@author: david
'''

import os

import pyworkflow.tests as tests

from simple.protocols import ProtRecVol
from pyworkflow.em.data import SetOfMovies
from pyworkflow.em.protocol import ProtImportMovies

class TestRecVolBase(tests.BaseTest):
    """This class checks if the protocol to call Unblur
       works properly.
    """
    @classmethod
    def setUpClass(cls):
        tests.setupTestProject(cls)
        
class TestRecVol(TestRecVolBase):
    @classmethod    
    def setUpClass(cls):
        tests.setupTestProject(cls)
        cls.protImport = cls.runImportFromScipion()

    @classmethod
    def runImportFromScipion(cls):
        args = {'importFrom': ProtImportMovies.IMPORT_FROM_FILES,
                'filesPath': '/media/david/linux/Documentos/CNB/TFG/simpleData/simple2.5tutorials/3_PRIME3D/data/',
                'filesPattern': '*.mrc',
                'amplitudConstrast': 0.1,
                'sphericalAberration': 2.,
                'voltage': 300,
                'samplingRate': 2.68
                }
        prot1 = cls.newProtocol(ProtImportMovies,**args)
        prot1.setObjLabel('from files')
        cls.launchProtocol(prot1)
        return prot1
        
    def test_RecVol(self):
        prot2 = self.newProtocol(ProtRecVol, symmetry='c1',
                                 oritab='/home/david/ScipionUserData/projects/TestM2PC/mapped_ptcls_params.txt')
        prot2.inputMovies.set(self.protImport.outputMovies)
        self.launchProtocol(prot2)
        
        
